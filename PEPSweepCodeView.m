...
@interface PEPSweepCodeView ()

@property (nonatomic, strong) UIImageView *sweepView;

@end

@implementation PEPSweepCodeView
...
- (void)initSubViews {
    CGFloat scanContent_X = self.frame.size.width * 0.15;
    CGFloat scanContent_Y = self.frame.size.height * 0.24;
    if([[UIDevice currentDevice].model containsString:@"iPad"]) {
        scanContent_X = self.frame.size.width * 0.3;
        scanContent_Y = self.frame.size.height * 0.3;
    }
    // 扫描内容的创建
    ...
    self.sweepView.image = [UIImage imageNamed:kImageNameQRCodeSweepLine];
    self.sweepView.frame = CGRectMake(scanContent_X * 0.5, scanContent_layerY, self.frame.size.width - scanContent_X , 12);
    ...
    [light_button setTitle:@"闪光灯" forState:UIControlStateSelected];
    ...
    UIImage *left_image = [UIImage imageNamed:kImageNameQRCodeTopLeft];
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:kImageNameQRCodeTopRight];
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:kImageNameQRCodeBottomLeft];
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:kImageNameQRCodeBottmRight];
